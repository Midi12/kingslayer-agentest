  ...
  const groups = conveyors
    .map((conveyor, index) => {
      ...
      return `<g data-testid="svg-${idLower}" role="img" tabindex="0" data-conveyor="${escapeAttr(conveyor.id)}" style="cursor:pointer">
        <title>${escapeHtml(conveyor.id)} - ${escapeHtml(conveyor.status)}</title>
        ...
        <text x="${x + 10}" y="${y + 44}" fill="white" font-size="12" font-family="system-ui">${escapeHtml(conveyor.status)}</text>
      </g>`;
    })
    .join('\n');
  const script = `<script>
document.querySelectorAll('[data-conveyor]').forEach(function (g) {
  g.addEventListener('click', function () {
    ...
    fetch('/sim/conveyors/' + id + '/start', { method: 'POST' }).then(function () { location.reload(); });
  });
});
</script>`;
