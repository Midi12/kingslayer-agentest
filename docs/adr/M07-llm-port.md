...
The engine sees two error codes, mapped by `analystBreakReason` to `ANALYST_UNAVAILABLE` and `ANALYST_INVALID`. `AnalystError` (M01) has no usage member, so a failed operation cannot return the tokens its completed calls used (one call when the repair call fails, two when the answer stays invalid). `LlmAnalyst` therefore reports every completed provider call through the `onResponse` option (operation, attempt, model, input and output tokens, stop reason); M09 and M10 meter failed escalations, and fill `escalation.decided` `inputTokens`/`outputTokens` for `ANALYST_INVALID` outcomes, from it. A usage member on `AnalystError` would be a contracts change for M01. A new dialect (for example Bedrock) is a third adapter behind the same port and contract suite (M07-G6). The retry budget is a constant here; the brain (M09) can pass its own policy.
