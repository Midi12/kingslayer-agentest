...
M07 and M18 test their validators against a model that really misbehaves; the grammar of `malicious` is small, so an attack corpus that needs another shape of instruction adds a rule here.
