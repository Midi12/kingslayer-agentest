...
- `compose.dev.yaml` gains `fake-jev` and `fake-llm` under the profile `fakes` (loopback ports 4100 and 4200, hardened like every ARGUS service); the always-on services stay postgres and s3, and the M00 repository test now checks the always-on set.
