...
- Order of a call: authentication, JSON parse, validation, then the per-call outcome queue, then the mode. A queued outcome is `{ status?, retryAfterMs?, body?, delayMs?, hang?, answers? }`: an error response, a delay before responding, a request that never answers (the client times out), or answers for this call only. Queues exist per rule and globally; the first matching rule's queue is used first. The queue applies in every mode.
