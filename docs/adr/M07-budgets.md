...
- Budgets: triage 20,000 tokens. The report default is 12,000, below the 16,000 the M07 spec allows, because M19-G5 requires report input of at most 12,000 tokens; a caller may raise it up to 16,000. The ceilings are enforced, not advisory: `LlmAnalyst.create` refuses (does not clamp) limits above `LIMIT_CEILINGS` in `core/budgets.ts` (triage 20,000 tokens and six frames, report 16,000 tokens and four key frames, vision 20,000 tokens, long edge 1,280 px), limits that are not whole numbers, and a triage minimum above its maximum. A refused configuration is a wiring bug that should surface at start-up, not a request silently sent under different limits.
...
- Frames: those with an image, nearest the break first (equal distance: before the break first), at most six, sent in time order after being scaled by sharp through the `ImageScaler` port (`SharpImageScaler`, media type kept, never upscaled; an image within the size limit whose data is not in its declared format is re-encoded to that format rather than sent mislabelled). A frame that does not decode is left out. Fewer than three are sent only when fewer have images.
